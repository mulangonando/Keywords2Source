...
class model(object):
    ...
    def __init__(self, FLAGS):
        ...
        # Logits
        logits_u = rnn_softmax(FLAGS, outputs)
        probabilities = tf.nn.softmax(logits_u)
        self.accuracy = tf.equal(tf.argmax(
            self.targets_y,1), tf.argmax(logits_u,1))

        # Loss
        self.loss = tf.reduce_mean(
            #tf.nn.sigmoid_cross_entropy_with_logits(logits, self.targets_y))
            #Line to Replace the original one above : tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits_, labels=tf.ones_like(self.D_))
            tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_u, labels=self.targets_y))
    ...
    def step(self, sess, batch_X, batch_seq_lens, batch_y=None, dropout=0.0,
        ...
        if forward_only:
            ...
            print (item.next() for item in output_feed)
