...
from data_handler import (
    ...
    get_char_tokens,
    generate_epoch,
    load_char_data_and_labels,
    #create_vocabulary,
    create_char_vocabulary,
    data_to_character_ids,
    #data_to_token_ids,
    split_data,
)
...
tf.app.flags.DEFINE_integer("en_vocab_size", 258,
...
def train():
    ...
    with tf.Session() as sess:
        ...
        # Train results
        returned  = generate_epoch(train_X, train_y,
            ...
            for valid_epoch_num, valid_epoch in enumerate(
                ...
                for valid_batch_num, \
                    ...
                    loss, accuracy = model.step(sess,
                        ...
                        dropout=0.0, forward_only=False, sampling=False)
...
def sample():
    ...
    vocab_list, vocab_dict, rev_vocab_dict = create_char_vocabulary()
    X, seq_lens = data_to_character_ids(X, vocab_dict)

    #test_sentence = "It was the worst movie I have ever seen."
    #test_sentence = get_tokens(clean_str(test_sentence))
    test_sentence = "George Anyone"
    test_sentence = get_char_tokens(test_sentence)
    test_sentence, seq_len = data_to_character_ids([test_sentence], vocab_dict)
